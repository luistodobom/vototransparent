...
def run_pipeline(start_year=None, end_year=None, max_sessions_to_process=None):
    ...
    if max_sessions_to_process is not None and len(sessions_to_process_infos) > max_sessions_to_process:
        print(f"Limiting processing to {max_sessions_to_process} sessions due to max_sessions_to_process limit.")
    ...
    def _process_single_session(session_info, df_obj, lock_obj, session_pdf_dir, proposal_doc_dir, 
        ...
        with lock_obj:
            existing_rows_for_session_pdf = df_obj[df_obj['session_pdf_url'] == current_session_pdf_url]
            ...
            if download_success:
                ...
                with lock_obj:
                    ...
                    if placeholder_indices.empty:
                        new_idx = len(df_obj)
                        df_obj.loc[new_idx, 'session_pdf_url'] = current_session_pdf_url
                    ...
                    df_obj.loc[new_idx, 'session_pdf_download_status'] = session_pdf_download_status_for_df
                    df_obj.loc[new_idx, 'last_error_message'] = session_pdf_download_error_for_df
                    df_obj.loc[new_idx, 'overall_status'] = 'Failed Stage 1 (Session PDF Download)'
                    df_obj.loc[new_idx, 'last_processed_timestamp'] = datetime.now().isoformat()
                    ...
                    for idx_other in other_indices:
                        df_obj.loc[idx_other, 'session_pdf_download_status'] = session_pdf_download_status_for_df
                        df_obj.loc[idx_other, 'last_error_message'] = session_pdf_download_error_for_df
                        df_obj.loc[idx_other, 'overall_status'] = 'Failed Stage 1 (Session PDF Download)'
                        df_obj.loc[idx_other, 'last_processed_timestamp'] = datetime.now().isoformat()
        ...
        if session_parse_error_for_df or (session_parse_status_for_df == 'LLM Parsed - No Propostas Encontradas' and not proposals_from_llm) :
            with lock_obj:
                ...
                if not summary_row_indices.empty:
                    ...
                    df_obj.loc[summary_idx_to_update, 'session_pdf_url'] = current_session_pdf_url
                    for col in columns_func():
                        if col not in ['session_pdf_url']: df_obj.loc[summary_idx_to_update, col] = pd.NA

                df_obj.loc[summary_idx_to_update, 'session_year'] = session_year
                df_obj.loc[summary_idx_to_update, 'session_date'] = session_date
                df_obj.loc[summary_idx_to_update, 'session_pdf_text_path'] = actual_session_pdf_disk_path
                df_obj.loc[summary_idx_to_update, 'session_pdf_download_status'] = 'Success' 
                df_obj.loc[summary_idx_to_update, 'session_parse_status'] = session_parse_status_for_df
                df_obj.loc[summary_idx_to_update, 'last_error_message'] = session_parse_error_for_df 
                df_obj.loc[summary_idx_to_update, 'overall_status'] = 'Failed Stage 2 (LLM Session Parse)' if session_parse_error_for_df else 'Completed (No Propostas)'
                df_obj.loc[summary_idx_to_update, 'last_processed_timestamp'] = datetime.now().isoformat()
        ...
        if proposals_from_llm is None or (not proposals_from_llm and not run_stage2_llm_parse): 
            with lock_obj:
                ...
                if not summary_row_indices.empty:
                    ...
                    current_overall_status_val = df_obj.loc[summary_idx, 'overall_status']
                    is_terminal = pd.notna(current_overall_status_val) and current_overall_status_val in terminal_statuses
                    if pd.isna(current_overall_status_val) or not is_terminal:
                        df_obj.loc[summary_idx, 'overall_status'] = 'Completed (No Propostas)' 
                        df_obj.loc[summary_idx, 'session_parse_status'] = session_parse_status_for_df 
                        df_obj.loc[summary_idx, 'last_processed_timestamp'] = datetime.now().isoformat()
                else: 
                    summary_idx = len(df_obj)
                    df_obj.loc[summary_idx, 'session_pdf_url'] = current_session_pdf_url
                    df_obj.loc[summary_idx, 'session_year'] = session_year
                    df_obj.loc[summary_idx, 'session_date'] = session_date
                    df_obj.loc[summary_idx, 'session_pdf_text_path'] = actual_session_pdf_disk_path
                    df_obj.loc[summary_idx, 'session_pdf_download_status'] = 'Success'
                    df_obj.loc[summary_idx, 'session_parse_status'] = session_parse_status_for_df
                    df_obj.loc[summary_idx, 'overall_status'] = 'Completed (No Propostas)'
                    df_obj.loc[summary_idx, 'last_processed_timestamp'] = datetime.now().isoformat()
                save_dataframe(df_obj)
        ...
        for proposal_data_from_llm in proposals_from_llm:
            ...
            approval_status_from_llm = proposal_data_from_llm.get('proposal_approval_status')
            ...
            with lock_obj:
                proposal_row_match_indices = df_obj[
                    ...
                    ( (df_obj['proposal_gov_link'] == proposal_gov_link) if pd.notna(proposal_gov_link) else df_obj['proposal_gov_link'].isna() )
                ].index
                
                row_idx = -1
                if proposal_row_match_indices.empty:
                    row_idx = len(df_obj) 
                    df_obj.loc[row_idx, 'session_pdf_url'] = current_session_pdf_url
                    df_obj.loc[row_idx, 'session_year'] = session_year 
                    df_obj.loc[row_idx, 'proposal_name_from_session'] = proposal_name
                    for col in columns_func():
                ...
                df_obj.loc[row_idx, 'session_pdf_text_path'] = actual_session_pdf_disk_path
                df_obj.loc[row_idx, 'session_pdf_download_status'] = 'Success' 
                df_obj.loc[row_idx, 'proposal_gov_link'] = proposal_gov_link
                df_obj.loc[row_idx, 'voting_details_json'] = json.dumps(voting_summary) if voting_summary else None
                df_obj.loc[row_idx, 'session_parse_status'] = session_parse_status_for_df 
                df_obj.loc[row_idx, 'proposal_approval_status'] = approval_status_from_llm
                
                current_overall_status = df_obj.loc[row_idx, 'overall_status']
                is_current_overall_status_terminal = pd.notna(current_overall_status) and current_overall_status in terminal_statuses
                
                if pd.isna(current_overall_status) or not is_current_overall_status_terminal:
                     df_obj.loc[row_idx, 'overall_status'] = 'Pending Further Stages'
                     df_obj.loc[row_idx, 'last_error_message'] = pd.NA 
                     df_obj.loc[row_idx, 'proposal_details_scrape_status'] = pd.NA
            ...
            if pd.notna(proposal_gov_link) and isinstance(proposal_gov_link, str) and proposal_gov_link.startswith("http"):
                current_scrape_status = df_obj.loc[row_idx, 'proposal_details_scrape_status']
                ...
                if str(session_date) == str(last_processed_date_in_csv): # Compare current session's date
                ...
                current_overall_status_for_else = df_obj.loc[row_idx, 'overall_status']
                ...
                if update_overall_status_to_no_gov_link:
                    df_obj.loc[row_idx, 'overall_status'] = 'Completed (No Gov Link for Details)'
                df_obj.loc[row_idx, 'proposal_details_scrape_status'] = 'No Gov Link'
            ...
            if needs_stage3_run:
                ...
                df_obj.loc[row_idx, 'proposal_authors_json'] = details_result['authors_json']
                df_obj.loc[row_idx, 'proposal_document_url'] = details_result['document_info']['link']
                df_obj.loc[row_idx, 'proposal_document_type'] = details_result['document_info']['type']
                ...
                df_obj.loc[row_idx, 'proposal_details_scrape_status'] = details_result['scrape_status']
                
                if details_result['error'] and \
                   (pd.isna(details_result['scrape_status']) or details_result['scrape_status'] != 'Success (No Doc Link)'): 
                    df_obj.loc[row_idx, 'last_error_message'] = str(details_result['error'])
                    df_obj.loc[row_idx, 'overall_status'] = 'Failed Stage 3 (Proposal Details Scrape)'
            ...
            doc_dl_status_s4 = df_obj.loc[row_idx, 'proposal_doc_download_status']
            ...
            if doc_is_successful_s4 and \
               ...
               not overall_status_s4_str.startswith('Failed Stage 3'):
                ...
                current_summary_status_s4 = df_obj.loc[row_idx, 'proposal_summarize_status']
            ...
            if needs_stage4_run:
                proposal_doc_disk_path_for_summary = df_obj.loc[row_idx, 'proposal_document_local_path']
                ...
                if summary_err:
                    df_obj.loc[row_idx, 'proposal_summarize_status'] = f'LLM Summary Failed: {summary_err}'
                    df_obj.loc[row_idx, 'last_error_message'] = summary_err
                    df_obj.loc[row_idx, 'overall_status'] = 'Failed Stage 4 (LLM Summary)'
                else:
                    df_obj.loc[row_idx, 'proposal_summary_general'] = summary_data['general_summary']
                    df_obj.loc[row_idx, 'proposal_summary_analysis'] = summary_data['critical_analysis']
                    df_obj.loc[row_idx, 'proposal_summary_fiscal_impact'] = summary_data['fiscal_impact']
                    df_obj.loc[row_idx, 'proposal_summary_colloquial'] = summary_data['colloquial_summary']
                    df_obj.loc[row_idx, 'proposal_category'] = summary_data['categories'] 
                    df_obj.loc[row_idx, 'proposal_short_title'] = summary_data['short_title'] 
                    df_obj.loc[row_idx, 'proposal_proposing_party'] = summary_data['proposing_party']
                    df_obj.loc[row_idx, 'proposal_summarize_status'] = 'Success'
            ...
            if is_pending_for_final_update:
                summarize_status_val = df_obj.loc[row_idx, 'proposal_summarize_status']
                is_summarize_success = pd.notna(summarize_status_val) and summarize_status_val == 'Success'
                
                doc_dl_status_final = df_obj.loc[row_idx, 'proposal_doc_download_status']
                details_scrape_status_final = df_obj.loc[row_idx, 'proposal_details_scrape_status']
                ...
                if is_summarize_success:
                    ...
                    if doc_not_success_final and details_scrape_is_success_variant_final:
                        df_obj.loc[row_idx, 'overall_status'] = 'Completed (No Proposal Doc to Summarize)'
                    elif details_scrape_is_no_gov_link_final:
                         df_obj.loc[row_idx, 'overall_status'] = 'Completed (No Gov Link for Details)'

            df_obj.loc[row_idx, 'last_processed_timestamp'] = datetime.now().isoformat()
            save_dataframe(df_obj) 
    ...
    with ThreadPoolExecutor(max_workers=25) as executor:
        ...
        results = list(executor.map(lambda p: _process_single_session(*p), starmap_args))
